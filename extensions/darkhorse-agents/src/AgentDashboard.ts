...
import { AgentOrchestrator, AgentRecord, AgentStatus } from './AgentOrchestrator';

export class AgentDashboard implements vscode.Disposable {

  public static readonly VIEW_TYPE = 'darkhorse.agentDashboard';
  private static instance: AgentDashboard | undefined;
  ...
  private constructor(
    ...
    this.refreshTimer = setInterval(() => {
      if (this.hasRunningAgents()) { this.pushUpdate(); }
    }, 3000);
  }
  ...
  private handleMessage(msg: DashboardMessage): void {
    switch (msg.type) {
      case 'terminate':
        if (msg.agentId) { this.orchestrator.terminate(msg.agentId); }
        break;
      case 'clearCompleted':
        ...
        break;
      case 'viewLog':
        if (msg.agentId) { this.openAgentLog(msg.agentId); }
        break;
    }
  }

  private async openAgentLog(agentId: string): Promise<void> {
...
interface DashboardMessage {
  type:     'terminate' | 'clearCompleted' | 'viewLog' | 'viewReport';
  agentId?: string;
}

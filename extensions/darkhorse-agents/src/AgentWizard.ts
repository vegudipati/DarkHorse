...
import { AgentConfig, AgentPermission, AgentOrchestrator } from './AgentOrchestrator';
...
export class AgentWizard {
  ...
  public static readonly VIEW_TYPE = 'darkhorse.agentWizard';
  ...
  public static async show(
    ...
    return new Promise<WizardResult>((resolve) => {
      ...
      panel.onDidDispose(() => {
        resolve({ config: {} as AgentConfig, cancelled: true });
      });
    });
  }
  ...
  private constructor(
    ...
    private resolve: (result: WizardResult) => void
  ) {
    this.panel.webview.onDidReceiveMessage((msg: WizardMessage) => {
  ...
  private handleMessage(msg: WizardMessage): void {
    switch (msg.type) {
      case 'launch':
      ...
      case 'cancel':
        this.panel.dispose();
        this.resolve({ config: {} as AgentConfig, cancelled: true });
        break;
      case 'getActiveFile':
  ...
  private handleLaunch(formData: WizardFormData): void {
    const permissions: AgentPermission[] = ['read_sap', 'call_llm', 'read_files'];
    if (formData.allowWrite) {
      permissions.push('write_sap');
    }

    const config: AgentConfig = {
      agentId:         AgentOrchestrator.generateId(formData.agentType),
...
interface WizardMessage {
  type:      'launch' | 'cancel' | 'getActiveFile';

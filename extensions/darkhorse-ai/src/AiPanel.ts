...
// Message types flowing between the webview and the extension host
interface WebviewMessage {
  type: 'generate' | 'ready' | 'cancel';
  prompt?: string;
  sessionId?: string;
}

interface ExtensionMessage {
  type: 'response' | 'error' | 'loading' | 'context';
  code?: string;
  explanation?: string;
  model?: string;
  tokensUsed?: number;
...
export class AiPanel implements vscode.Disposable {
  ...
  public static readonly VIEW_TYPE = 'darkhorse.aiPanel';
  ...
  private constructor(panel: vscode.WebviewPanel, proxyManager: ProxyManager) {
    this.panel          = panel;
    ...
    this.disposables.push(
      vscode.window.onDidChangeActiveTextEditor(() => this.pushContextToWebview())
    );
  ...
  private async handleGenerate(prompt: string, sessionId?: string): Promise<void> {
    ...
    // Call the proxy
    let result: { code: string; explanation?: string; model: string; tokensUsed: number };
    try {
  ...
  private async pushContextToWebview(): Promise<void> {
    const context = this.contextCollector.collect();
    this.postMessage({
      type: 'context',
      context: {

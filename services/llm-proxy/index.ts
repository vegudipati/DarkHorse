...
const BACKEND    = (process.env.DARKHORSE_LLM_BACKEND || 'claude') as 'claude' | 'ollama';

// Audit log written to user's home dir so it survives restarts
const AUDIT_LOG_PATH = path.join(
  process.env.DARKHORSE_AUDIT_LOG_DIR || os.homedir(),
  '.darkhorse',
  'llm-audit.jsonl'
);
...
function validateEnvironment(): void {
  if (BACKEND === 'claude' && !API_KEY) {
    ...
    process.stderr.write(
      '[DarkHorse Proxy] FATAL: DARKHORSE_CLAUDE_API_KEY is not set. ' +
  ...
  if (isNaN(PROXY_PORT) || PROXY_PORT < 1024 || PROXY_PORT > 65535) {
    process.stderr.write(
      `[DarkHorse Proxy] FATAL: Invalid port ${PROXY_PORT}. Must be 1024-65535.\n`
    );
    process.exit(1);
  }
}
...
function buildApp(
  ...
  app.post('/review', async (req: express.Request, res: express.Response) => {
    ...
    const body = req.body as { code?: string; objectName?: string; sessionId?: string };
  ...
  // ------------------------------------------------------------------
  app.post('/explain', async (req: express.Request, res: express.Response) => {
    const startMs = Date.now();
    ...
    const body = req.body as { code?: string; sessionId?: string };
    if (!body.code || typeof body.code !== 'string') {
      res.status(400).json({ error: 'code is required.' });
      return;
    }
...
// Graceful shutdown
// ---------------------------------------------------------------------------
function setupShutdownHandlers(server: http.Server, auditLogger: AuditLogger): void {
...
async function main(): Promise<void> {
  ...
  const rateLimiter = new RateLimiter(RATE_LIMIT_MAX_TOKENS, RATE_LIMIT_REFILL_RATE);
  const auditLogger = new AuditLogger(AUDIT_LOG_PATH);
  const systemPrompt = AbapSystemPrompt.get();
  ...
  if (BACKEND === 'ollama') {
    ...
    adapter = new OllamaAdapter(ollamaUrl, ollamaModel, systemPrompt);
    process.stdout.write(`[DarkHorse Proxy] Using Ollama backend at ${ollamaUrl}, model: ${ollamaModel}\n`);
  } else {
    // Default: Claude
    adapter = new ClaudeAdapter(API_KEY, systemPrompt);
  ...
  server.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE') {
      process.stderr.write(
        `[DarkHorse Proxy] FATAL: Port ${PROXY_PORT} is already in use. ` +

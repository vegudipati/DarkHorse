...
export class ClaudeAdapter implements ILLMAdapter {
  ...
  constructor(
    ...
    private readonly systemPromptOverride?: string  // used for testing
  ) {}
  ...
  private async callClaude(systemPrompt: string, userMessage: string): Promise<GenerateResponse> {
    const body = {
      model: MODEL,
...
// ---------------------------------------------------------------------------
// Anthropic API response shape (minimal — only what we use)
// ---------------------------------------------------------------------------
